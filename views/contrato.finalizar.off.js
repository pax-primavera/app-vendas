...
import { cnpjMask } from "../utils/generic/format";
...
function ContratoContentFinalizarOff({ navigation }) {
  ...
  const setup = async () => {
    ...
    executarSQL(`select regiao, uf from unidade where id = ${unidadeID}`).then((response) => {
      setRegiao(response._array[0].regiao)
  ...
  const finalizarContrato = async () => {
    ...
    try {
      ...
      from plano where id = "${contratoTratado.plano}" and unidadeId = ${unidadeID}`);
  ...
  <body>
      ...
      <body>
          ...
          <div>
              ...
              <p><span style="color: inherit;">Naturalidade: <span
                          class="NATURALIDADE_TITULAR token_d4s">${contratoCliente.naturalidadeTitular}</span>&nbsp;</span>q
              </p>
...
<html lang="pt-br">
  ...
  <body>
    ...
    <div>
      ...
      return navigation.navigate("contratoContentAssinatura", { contratoID, anexos, id, html });
    } catch (e) {
      setCarregamentoButton(false);
      toast.show({
        placement: "top",
