...
function ContratoContentFinalizarOff({ navigation }) {
    /// Config
    ...
    const finalizarContratoOff = async () => {
                ...
                [
                    {
                        ...
                        onPress: async () => {
                            return navigation.navigate("VendasPendentes")
                        }
                    },
    ...
    const finalizarContrato = async () => {
        ...
        try {
            ...
            const contrato = await executarSQL(`select * from titulares where id = '${id}'`);
          ...
          `);
            ...
            contratoBody.append("anexos[]", JSON.parse(contratoTratado.anexo1));
            contratoBody.append("anexos[]", JSON.parse(contratoTratado.anexo2));
            contratoBody.append("anexos[]", JSON.parse(contratoTratado.anexo3));
            contratoBody.append("anexos[]", JSON.parse(contratoTratado.anexo4));
            contratoBody.append("anexos[]", JSON.parse(contratoTratado.anexo5));
            contratoBody.append("anexos[]", JSON.parse(contratoTratado.anexo6));
            ...
            if (request.data && request.data.assinatura) {
                ...
                await executarSQL(`
                    DELETE from dependentes WHERE titular_id = ${id}`
                );

                await executarSQL(`
                    DELETE from titulares WHERE id = ${id}`
                );
    ...
    const finalizar = async () => {
        Alert.alert(
            "Aviso.",
            "Deseja 'FINALIZAR' o contrato no modo 'ONLINE' ou 'SALVAR' localmente? \n\n"+
            "Ao selecionar 'SALVAR' você será redirecionado para a tela de Vendas Pendentes podendo 'ALTERAR O DADOS'.\n"+
            "Ao selecionar 'ONLINE' você será redirecionado para a tela de assinatura do contrato.\n\n"+
            "REVISE OS DADOS POR GARANTIA!",
            [
                {
                    text: "SALVAR",
                    
                    onPress: async () => {
                        finalizarContratoOff()
                    }
                },
                {
                    text: " ONLINE",
                        ...
                        <Box key="1" safeArea w="100%" pl="5" pr="5" mb="2" pb="5" maxW="100%" rounded="lg" overflow="hidden" borderColor="coolGray.200" borderWidth="1" _light={light} _web={web} >
                            ...
                            <HStack space={2} justifyContent="center">
                                <Center w="100%" rounded="md">
                                    <FormControl>
                                        <FormControl.Label>Selecione um modelo de contrato:</FormControl.Label>
                                        <Radio.Group
...
export { ContratoContentFinalizarOff }
